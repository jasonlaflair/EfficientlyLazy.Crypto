...
{
    public partial class TripleDESEngineControl : CryptoUserControl
    {
        public TripleDESEngineControl()
        {
            InitializeComponent();

            cmbKeySize.DataSource = EnumerationConversions.GetEnumDescriptions(typeof(TripleDESKeySize));
            cmbKeySize.SelectedItem = EnumerationConversions.GetEnumDescription(TripleDESKeySize.Key192Bit);
        ...
        public override string DisplayName { get { return "TripleDES Encryption/Decryption"; } }

        private void TripleDESEngineControl_Load(object sender, EventArgs e)
        {
        ...
        {
            var engine = new TripleDESEngine(txtKey.Text);

            if (cbxUseKeySize.Checked)
            {
                var keySize = EnumerationConversions.GetEnumName<TripleDESKeySize>(cmbKeySize.SelectedItem.ToString());
