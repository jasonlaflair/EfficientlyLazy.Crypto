...
{
    public partial class RC2EngineControl : CryptoUserControl
    {
        public RC2EngineControl()
        {
            InitializeComponent();

            cmbKeySize.DataSource = EnumerationConversions.GetEnumDescriptions(typeof(RC2KeySize));
            cmbKeySize.SelectedItem = EnumerationConversions.GetEnumDescription(RC2KeySize.Key128Bit);
        ...
        public override string DisplayName { get { return "RC2 Encryption/Decryption"; } }

        private void RC2EngineControl_Load(object sender, EventArgs e)
        {
        ...
        {
            var engine = new RC2Engine(txtKey.Text);

            if (cbxUseKeySize.Checked)
            {
                var keySize = EnumerationConversions.GetEnumName<RC2KeySize>(cmbKeySize.SelectedItem.ToString());
